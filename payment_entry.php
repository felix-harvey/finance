...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_payment') {
        ...
        $type = $_POST['type'] ?? ''; // 'Receive' or 'Make'
        $reference_number = $_POST['reference_number'] ?? '';
        $notes = $_POST['notes'] ?? '';
        $budget_source = $_POST['budget_source'] ?? ''; 

        if (empty($contact_id) || empty($amount) || empty($type) || empty($budget_source)) {
        ...
        try {
            // === LOGIC CHANGE: Send 'Receive' (AR) to Approval Queue ===
            if ($type === 'Receive') {
                
                // 1. Ensure 'disbursement_requests' can handle transaction types
                try {
                    $db->query("SELECT transaction_type FROM disbursement_requests LIMIT 1");
                } catch (PDOException $e) {
                    // Add column to distinguish 'Make' vs 'Receive'
                    $db->exec("ALTER TABLE disbursement_requests ADD COLUMN transaction_type VARCHAR(20) DEFAULT 'Make'");
                    $db->exec("ALTER TABLE disbursement_requests ADD COLUMN invoice_id INT NULL");
                    $db->exec("ALTER TABLE disbursement_requests ADD COLUMN contact_id INT NULL");
                    $db->exec("ALTER TABLE disbursement_requests ADD COLUMN payment_method VARCHAR(50) NULL");
                    $db->exec("ALTER TABLE disbursement_requests ADD COLUMN budget_source VARCHAR(100) NULL");
                }

                // 2. Generate Request ID
                $req_year = date('Y');
                $count = $db->query("SELECT COUNT(*) FROM disbursement_requests")->fetchColumn();
                $request_id = "REQ-" . $req_year . "-" . str_pad((string)($count + 1), 4, '0', STR_PAD_LEFT);

                // 3. Prepare Description
                $contactName = $db->query("SELECT name FROM business_contacts WHERE id = $contact_id")->fetchColumn();
                $desc = " $contactName " . $notes;

                // 4. Insert into Approval Queue
                $stmt = $db->prepare("INSERT INTO disbursement_requests 
                    (request_id, requested_by, department, description, amount, status, date_requested, 
                     invoice_id, contact_id, payment_method, budget_source, transaction_type) 
                    VALUES (?, ?, ?, ?, ?, 'Pending', NOW(), ?, ?, ?, ?, 'Receive')");
                
                $stmt->execute([
                    $request_id,
                    $user_id,
                    $budget_source, // Department is now the selected budget source
                    $desc,
                    $amount,
                ...
                $_SESSION['success'] = "Collection queued for approval (ID: $request_id). Please wait for admin approval.";
                header("Location: pending_disbursements.php");
                exit;
            } 
            
            // If 'Make' (AP), process normally
            else {
                // Generate payment ID for Make payments
                $payment_id = generatePaymentId($db, 'Make');
                ...
                $sql = "INSERT INTO payments (payment_id, contact_id, invoice_id, payment_date, amount, 
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, 'Make', ?, ?, 'Completed')";
                $stmt = $db->prepare($sql);
                $stmt->execute([$payment_id, $contact_id, !empty($invoice_id) ? $invoice_id : null, 
            ...
            <form id="payment-form" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="form-group">
                        ...
                        <select name="budget_source" class="form-input" required>
