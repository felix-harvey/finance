...
// Helper function to safely output HTML with null checks
...
<body class="bg-gray-bg">
    ...
    <div id="profile-modal" class="modal">
        <div class="modal-content">
            ...
            <div class="flex items-center mb-6">
                ...
                <div class="ml-4">
                    <h3 class="text-lg font-bold" id="profile-name"><?php echo safeOutput($user['name']); ?></h3>
                    <p class="text-gray-500"><?php echo ucfirst(safeOutput($user['role'])); ?></p>
                </div>
            </div>
            <div class="space-y-4">
                <div>
            ...
            <div class="bg-primary-green text-white p-4 flex justify-between items-center">
                ...
                <div class="flex items-center space-x-3">
                        ...
                        <div id="notification-dropdown" class="notification-dropdown">
                            ...
                            <div id="notification-list">
                                <?php if (count($notifications) > 0): ?>
                                    ...
                                    <div class="notification-item <?php echo (!isset($notification['read']) || !$notification['read']) ? 'unread' : ''; ?>" onclick="window.location.href='<?php echo safeOutput($notification['link']); ?>'">
                                        <div class="notification-message">
                                            <?php echo safeOutput($notification['message']); ?>
                                        </div>
                    ...
                    <div id="profile-btn" class="flex items-center space-x-2 cursor-pointer px-3 py-2 transition duration-200">
                        ...
                        <span class="text-white font-medium"><?php echo safeOutput($user['name']); ?></span>
            ...
            <div class="p-6 flex-1">
                ...
                <?php if (!empty($success_message)): ?>
                    <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-6">
                        <?php echo safeOutput($success_message); ?>
                    </div>
                <?php endif; ?>
                ...
                <div class="bg-white rounded-xl p-6 card-shadow mb-6">
                    ...
                    <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="start_date" class="form-input" value="<?php echo safeOutput($start_date); ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label">End Date</label>
                            <input type="date" name="end_date" class="form-input" value="<?php echo safeOutput($end_date); ?>">
                        </div>
                                ...
                                <tbody>
                                    <?php if (count($invoices) > 0): ?>
                                        ...
                                        <tr class="<?php echo $row_class; ?>">
                                            <td class="font-mono font-medium"><?php echo safeOutput($invoice['invoice_number'] ?? ''); ?></td>
                                            <td>
                                                <div class="font-medium"><?php echo safeOutput($invoice['contact_name'] ?? 'N/A'); ?></div>
                                                <div class="text-sm text-gray-500"><?php echo safeOutput($invoice['contact_person'] ?? ''); ?></div>
                                            </td>
                                            <td>
                                            ...
                                            <td><?php echo safeOutput($invoice['issue_date'] ?? ''); ?></td>
                                            <td>
                                                <div class="<?php echo $is_overdue ? 'text-red-600 font-medium' : ''; ?>">
                                                    <?php echo safeOutput($invoice['due_date'] ?? ''); ?>
                                                </div>
                                            </td>
                                            <td class="font-medium <?php echo !$_SESSION['show_numbers'] ? 'hidden-numbers' : ''; ?>">
                                                <?php echo formatNumber((float)($invoice['amount'] ?? 0), $_SESSION['show_numbers']); ?>
                                            </td>
                                            <td class="amount-positive <?php echo !$_SESSION['show_numbers'] ? 'hidden-numbers' : ''; ?>">
                                                <?php echo formatNumber((float)($invoice['amount_paid'] ?? 0), $_SESSION['show_numbers']); ?>
                                            </td>
                                            <td class="amount-negative <?php echo !$_SESSION['show_numbers'] ? 'hidden-numbers' : ''; ?>">
                                                <?php echo formatNumber((float)($invoice['outstanding_balance'] ?? 0), $_SESSION['show_numbers']); ?>
                                            </td>
                                            <td>
                                                <?php
                                                ...
                                                <span class="status-badge <?php echo $status_class; ?>">
                                                    <?php echo safeOutput($status); ?>
                                                </span>
                                            </td>
                                            <td>
                                                <div class="flex flex-wrap gap-2">
                                                    <?php if (($invoice['outstanding_balance'] ?? 0) > 0 && ($invoice['status'] ?? '') !== 'Cancelled'): ?>
                                                    <button class="action-btn pay" title="Record Payment" onclick="recordPayment(<?php echo (int)($invoice['id'] ?? 0); ?>)">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // FIX: Set due date to Fiscal Year End (Dec 31 of current year) by default
        const dueDateInput = document.querySelector('input[name="due_date"]');
        if (dueDateInput && !dueDateInput.value) {
            const today = new Date();
            const currentYear = today.getFullYear();
            
            // Set value to YYYY-12-31
            dueDateInput.value = `${currentYear}-12-31`;
        }
    });
    ...
    function updateContactOptions(type) {
        ...
        if (type === 'Receivable') {
            ...
            // FIX: Use 'let' or 'var' for all variables inside loops
            var customerName = '<?php echo addslashes($customer['name'] ?? ''); ?>';
            var customerId = '<?php echo $customer['contact_id'] ?? ''; ?>';
            if (customerName && customerId) {
                contactSelect.innerHTML += `<option value="${customerId}">${customerName} (${customerId})</option>`;
            }
            <?php endforeach; ?>
        }
        <?php endif; ?>
        ...
        if (type === 'Payable') {
            ...
            // FIX: Use 'let' or 'var'
            var vendorName = '<?php echo addslashes($vendor['name'] ?? ''); ?>';
            var vendorId = '<?php echo $vendor['contact_id'] ?? ''; ?>';
            if (vendorName && vendorId) {
                contactSelect.innerHTML += `<option value="${vendorId}">${vendorName} (${vendorId})</option>`;
            }
            <?php endforeach; ?>
        }
        <?php endif; ?>
        ...
        if (!type) {
            ...
            // FIX: Use 'let' or 'var'
            var contactName = '<?php echo addslashes($contact['name'] ?? ''); ?>';
            var contactId = '<?php echo $contact['contact_id'] ?? ''; ?>';
            var contactType = '<?php echo $contact['type'] ?? ''; ?>';
            if (contactName && contactId) {
                contactSelect.innerHTML += `<option value="${contactId}">${contactName} (${contactId}) - ${contactType}</option>`;
            }
            <?php endforeach; ?>
        }
        <?php endif; ?>
    ...
    // FIX: Attach to window so the button can always find it
    window.recordPayment = function(invoiceId) {
        // 1. Get the global event object safely
        const event = window.event;
        
        // 2. Robustly find the button
        const btn = event ? event.target.closest('button') : null;
        
        // 3. Add loading state if button was found
        if (btn) {
    ...
    function deleteInvoice(invoiceId, invoiceNumber) {
        if (confirm(`Are you sure you want to delete invoice "${invoiceNumber}"? This action cannot be undone.`)) {
            ...
            csrfInput.value = '<?php echo safeOutput($_SESSION['csrf_token']); ?>';
