...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'reject_with_reason') {
        $request_id = trim($_POST['request_id'] ?? '');
        $rejection_reason = trim($_POST['rejection_reason'] ?? '');
        
        if (empty($request_id) || empty($rejection_reason)) {
            $_SESSION['error'] = "Please provide a rejection reason.";
            header("Location: " . $_SERVER['PHP_SELF']);
            exit;
        }
        
        updateDisbursementStatus($db, $request_id, 'Rejected', $user_id, $rejection_reason);
    }
}
...
if (isset($_GET['action']) && isset($_GET['request_id'])) {
    if ($_GET['action'] === 'approve') {
        ...
        // Show reject reason modal instead of immediate rejection
...
function updateDisbursementStatus(PDO $db, string $request_id, string $status, int $user_id, string $rejection_reason = ''): void {
    try {
        ...
        if ($status === 'Rejected' && !empty($rejection_reason)) {
            $stmt = $db->prepare("UPDATE disbursement_requests SET status = ?, date_approved = NOW(), approved_by = ?, rejection_reason = ? WHERE request_id = ?");
            $stmt->execute([$status, $user_id, $rejection_reason, $request_id]);
        } else {
        ...
        // Add notification for status update to database
        if ($status === 'Approved') {
            addNotificationToDatabase(
                ...
                'success',
                'Request Approved',
                "Disbursement request {$request_id} has been approved."
            );
            ...
            addNotificationToDatabase(
                ...
                'warning',
                'Request Rejected',
                "Disbursement request {$request_id} has been rejected. Reason: " . ($rejection_reason ?: 'No reason provided')
            );
...
<body class="bg-gray-bg">
    ...
    <div id="reject-reason-modal" class="modal">
        <div class="modal-content">
            <span class="close-modal">&times;</span>
            <h2 class="text-xl font-bold mb-4">Reject Disbursement Request</h2>
            <form id="reject-reason-form" method="POST">
                <input type="hidden" name="action" value="reject_with_reason">
                <input type="hidden" id="reject-request-id" name="request_id">
                
                <div class="form-group">
                    <label class="form-label">Rejection Reason</label>
                    <textarea name="rejection_reason" id="rejection-reason" class="form-input" rows="4" placeholder="Please provide a reason for rejecting this request..." required></textarea>
                </div>
                
                <div class="flex space-x-4 mt-6">
                    <button type="button" class="btn btn-secondary flex-1 close-modal">Cancel</button>
                    <button type="submit" class="btn btn-primary flex-1" style="background-color: #dc2626;">Reject Request</button>
                </div>
            </form>
        </div>
    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="py-2 mx-2">
                            ...
                            <div class="submenu active" id="disbursement-submenu">
                                <a href="disbursement_request.php" class="submenu-item">Disbursement Request</a>
                                <a href="pending_disbursements.php" class="submenu-item active">Pending Disbursements</a>
                                <a href="approved_disbursements.php" class="submenu-item">Approved Disbursements</a>
                                <a href="rejected_disbursements.php" class="submenu-item">Rejected Disbursements</a>
            ...
            <div class="bg-primary-green text-white p-4 flex justify-between items-center">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-white/90">Review and approve pending disbursement requests</p>
                    </div>
                </div>
        ...
        <button class="visibility-toggle" onclick="toggleAmountVisibility(this)">
            <i class="fa-solid fa-eye-slash"></i>
                                        ...
                                        <td>
                                            ...
                                            ?>
                                            <span class="status-badge status-pending"><?php echo $priority; ?></span>
                                        </td>
                                        <td>
                                            <div class="flex flex-wrap gap-2">
                                                <button class="action-btn approve" onclick="approveRequest('<?php echo $request['request_id']; ?>')">
                                                    <i class='bx bx-check mr-1'></i>Approve
                                                </button>
                                                <button class="action-btn reject" onclick="rejectRequest('<?php echo $request['request_id']; ?>')">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const rejectReasonForm = document.getElementById('reject-reason-form');
            if (rejectReasonForm) {
                rejectReasonForm.addEventListener('submit', function(e) {
                    const submitBtn = this.querySelector('button[type="submit"]');
                    const originalText = submitBtn.innerHTML;
                    
                    // Show loading state
                    submitBtn.innerHTML = '<div class="spinner"></div>Rejecting...';
                    submitBtn.disabled = true;
                });
            }
        ...
        function rejectRequest(requestId) {
            ...
            if (rejectReasonModal && rejectRequestId && rejectionReason) {
        ...
        // Edit request function (removed since edit button is removed)
        function editRequest(requestId) {
            // This function is no longer needed since edit button is removed
        }
