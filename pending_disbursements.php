...
if (isset($_GET['action']) && isset($_GET['request_id'])) {
    if ($_GET['action'] === 'approve') {
        updateDisbursementStatus($db, $_GET['request_id'], 'Approved', $user_id);
    } elseif ($_GET['action'] === 'delete') {
        deleteDisbursement($db, $_GET['request_id'], $user_id);
    }
}
...
function updateDisbursementStatus(PDO $db, string $request_id, string $status, int $user_id, string $rejection_reason = ''): void {
    try {
        $db->beginTransaction();

        // 1. Update Request Status (using 'Approved' for database compatibility)
        $dbStatus = 'Approved'; // Always use 'Approved' for database
        $stmt = $db->prepare("UPDATE disbursement_requests SET status = ?, date_approved = NOW(), approved_by = ? WHERE request_id = ?");
        $stmt->execute([$dbStatus, $user_id, $request_id]);

        // 2. Create the Actual Payment Record
        // Fetch request details
        $reqStmt = $db->prepare("SELECT * FROM disbursement_requests WHERE request_id = ?");
        $reqStmt->execute([$request_id]);
        $request = $reqStmt->fetch();

        if ($request) {
            // Determine Transaction Type (Default to Make if missing)
            $transType = $request['transaction_type'] ?? 'Make';
            
            // Generate Payment ID based on Type
            $prefix = ($transType === 'Receive') ? 'PMT' : 'V-PMT';
            $year = date('Y');
            $timestamp = time();
            $new_payment_id = "$prefix-$year-" . substr((string)$timestamp, -5);

            // Generate OR Number for Collections
            $or_number = '';
            if ($transType === 'Receive') {
                 // Simple OR generator logic
                 $or_number = "OR-" . date('Y') . "-" . substr((string)time(), -5);
            }

            // Insert into Payments Table
            $payStmt = $db->prepare("INSERT INTO payments 
                (payment_id, contact_id, invoice_id, payment_date, amount, payment_method, type, reference_number, notes, status) 
            ...
            $payStmt->execute([
                ...
                $transType, // 'Receive' or 'Make'
                $or_number, // OR Number if Receive
                $request['description']
            ]);

            // 3. Update Invoice Status (If linked)
            if (!empty($request['invoice_id'])) {
                $invSql = "SELECT amount FROM invoices WHERE id = ?";
                $invStmt = $db->prepare($invSql);
                $invStmt->execute([$request['invoice_id']]);
                $invAmount = $invStmt->fetchColumn();

                $paidSql = "SELECT SUM(amount) FROM payments WHERE invoice_id = ? AND status = 'Completed'";
                $paidStmt = $db->prepare($paidSql);
                $paidStmt->execute([$request['invoice_id']]);
                $totalPaid = $paidStmt->fetchColumn();

                if ($totalPaid >= $invAmount) {
                    $updStmt = $db->prepare("UPDATE invoices SET status = 'Paid' WHERE id = ?");
                    $updStmt->execute([$request['invoice_id']]);
                }
            }
        }
        ...
        // Notifications & Redirect - Use "disbursed" in the message for user
        $_SESSION['success'] = "Payment disbursed successfully!";
...
function editDisbursement(PDO $db, int $user_id): void {
    ...
    try {
        ...
        $checkStmt = $db->prepare("SELECT * FROM disbursement_requests WHERE request_id = ? AND status = 'Pending'");
        $checkStmt->execute([$request_id]);
        $request = $checkStmt->fetch();
        
        if (!$request) {
            $_SESSION['error'] = "You can only edit pending disbursement requests.";
            return;
        }
        
        // Update the request
        $stmt = $db->prepare("UPDATE disbursement_requests SET requested_by_name = ?, department = ?, description = ?, amount = ? WHERE request_id = ?");
        
        $stmt->execute([$requested_by, $department, $description, $amount, $request_id]);
        ...
        addNotificationToDatabase(
            ...
            "Disbursement request {$request_id} has been updated successfully."
        );
...
function deleteDisbursement(PDO $db, string $request_id, int $user_id): void {
    try {
        ...
        addNotificationToDatabase(
            ...
            "Disbursement request {$request_id} has been deleted."
        );
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="py-2 mx-2">
                            ...
                            <div class="submenu active" id="disbursement-submenu">
                                <a href="disbursement_request.php" class="submenu-item">Disbursement History</a>
                                <a href="pending_disbursements.php" class="submenu-item active">Pending Disbursements</a>
        ...
        <div id="main-content" class="flex-1 overflow-y-auto flex flex-col">
            ...
            <div class="bg-primary-green text-white p-4 flex justify-between items-center">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">Pending Disbursements</h1>
                        <p class="text-sm text-white/90">Review and disburse pending payment requests</p>
                    </div>
                </div>
            ...
            <div class="p-6 flex-1">
                ...
                <!-- Pending Disbursements Content -->
                <div class="bg-white rounded-xl p-6 card-shadow">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-xl font-bold text-dark-text">Pending Disbursements</h2>
                        <div class="text-sm text-gray-500">
                            Total: <?php echo count($pending_disbursements); ?> requests
                        </div>
                    </div>
                            ...
                            <tbody>
                                <?php if (count($pending_disbursements) > 0): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php 
                                                // Default to 'Make' if column doesn't exist yet
                                                $tType = $request['transaction_type'] ?? 'Make';
                                                $badgeClass = ($tType === 'Receive') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
                                                $labelText = ($tType === 'Receive') ? 'Collection (AR)' : 'Disbursement (AP)';
                                            ?>
                                        ...
                                        <td><?php echo htmlspecialchars($request['requested_by_name'] ?? $request['user_name'] ?? 'Unknown'); ?></td>
                                        <td><?php echo htmlspecialchars($request['department']); ?></td>
                                        
                                        <td class="font-semibold">
                                            <div class="amount-cell">
                                                <span class="amount-value hidden-amount" data-value="₱<?php echo number_format((float)$request['amount'], 2); ?>">********</span>
                                                <button class="visibility-toggle" onclick="toggleAmountVisibility(this)"><i class="fa-solid fa-eye-slash"></i></button>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="flex flex-wrap gap-2">
                                                <button class="action-btn disburse" onclick="disburseRequest('<?php echo $request['request_id']; ?>')">
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr><td colspan="8" class="text-center py-4 text-gray-500">No pending requests found.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (markAllReadBtn && !markAllReadBtn.disabled) {
                markAllReadBtn.addEventListener('click', function() {
                    ...
                    .then(data => {
                        if (data.success) {
                            // Refresh the page to load updated notifications from database
                            location.reload();
                        }
                    })
                    .catch(error => {
        ...
        function deleteRequest(requestId) {
            if (confirm('Are you sure you want to delete this disbursement request?\n\nThis action cannot be undone.')) {
                window.location.href = '?action=delete&request_id=' + requestId;
            }
        }

        function disburseRequest(requestId) {
            if (confirm('Are you sure you want to disburse this payment?')) {
                window.location.href = '?action=approve&request_id=' + requestId;
            }
        }

        // Function to attach event listeners to the edit form
        function attachEditFormListeners() {
            const editForm = document.getElementById('edit-disbursement-form');
            const closeButtons = document.querySelectorAll('.close-modal');
            ...
            closeButtons.forEach(button => {
                button.addEventListener('click', function() {
                    document.getElementById('edit-disbursement-modal').style.display = 'none';
                });
            });
        }
